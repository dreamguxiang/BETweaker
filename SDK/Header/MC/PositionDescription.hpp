// This Header is auto generated by BDSLiteLoader Toolchain
#pragma once
#define AUTO_GENERATED
#include "../Global.h"
#include "Json.hpp"

#define BEFORE_EXTRA

#undef BEFORE_EXTRA

struct PositionDescription {

#define AFTER_EXTRA

#undef AFTER_EXTRA
#ifndef DISABLE_CONSTRUCTOR_PREVENTION_POSITIONDESCRIPTION
public:
    struct PositionDescription& operator=(struct PositionDescription const &) = delete;
    PositionDescription(struct PositionDescription const &) = delete;
    PositionDescription() = delete;
#endif
public:
    /*0*/ virtual void __unk_vfn_0();
    /*1*/ virtual char const * getJsonName() const;
    /*2*/ virtual void __unk_vfn_2();
    /*3*/ virtual void getDocumentation(struct Documentation::Node &) const;
    /*4*/ virtual ~PositionDescription();
    /*5*/ virtual void deserializeData(class Json::Value &);
    /*6*/ virtual void serializeData(class Json::Value &) const;
#ifdef ENABLE_VIRTUAL_FAKESYMBOL_POSITIONDESCRIPTION
    MCVAPI char const * getDescription() const;
    MCVAPI char const * getNodeName() const;
#endif

};
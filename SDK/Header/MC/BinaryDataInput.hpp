// This Header is auto generated by BDSLiteLoader Toolchain
#pragma once
#define AUTO_GENERATED
#include "../Global.h"

#define BEFORE_EXTRA

#undef BEFORE_EXTRA

class BinaryDataInput {

#define AFTER_EXTRA

#undef AFTER_EXTRA
#ifndef DISABLE_CONSTRUCTOR_PREVENTION_BINARYDATAINPUT
public:
    class BinaryDataInput& operator=(class BinaryDataInput const &) = delete;
    BinaryDataInput(class BinaryDataInput const &) = delete;
    BinaryDataInput() = delete;
#endif

public:
    /*0*/ virtual ~BinaryDataInput();
    /*1*/ virtual void __unk_vfn_1();
    /*2*/ virtual void __unk_vfn_2();
    /*3*/ virtual float readFloat();
    /*4*/ virtual double readDouble();
    /*5*/ virtual char readByte();
    /*6*/ virtual short readShort();
    /*7*/ virtual int readInt();
    /*8*/ virtual __int64 readLongLong();
    /*9*/ virtual bool readBytes(void *, unsigned __int64);
    /*10*/ virtual unsigned __int64 numBytesLeft() const;
#ifdef ENABLE_VIRTUAL_FAKESYMBOL_BINARYDATAINPUT
    MCVAPI std::string readLongString();
    MCVAPI std::string readString();
#endif

};